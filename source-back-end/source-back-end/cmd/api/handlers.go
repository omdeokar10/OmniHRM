...
func (app *application) GetUserAttendance(w http.ResponseWriter, r *http.Request) {
	...
	// Fetch user attendance data based on the user_id
	// You should replace this with your actual data retrieval logic.
	// Here, we'll create a dummy UserAttendance for demonstration purposes.
	attendance := UserAttendance{
		Day: time.Now(),
		User: User{
			UserId: id,
		}, // You should populate the User struct
		UserPresent:       true,
		UserdateTimeStart: time.Now(),
		UserdateTimeEnd:   time.Now(),
	}

	// Serialize the UserAttendance struct to JSON
	jsonData, err := json.Marshal(attendance)
	if err != nil {
